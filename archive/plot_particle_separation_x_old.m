function p_h = plot_particle_separation_x(save_plot, multi_plot)
% plot horizontal separation (in x) between particles
...
for mm = 1:Np
    ...
    if mm == Np
        [time, sep, sep_vel] = particle_separation_x(mm-1, 0);
        sep = Lx - sep;
    else
        [time, sep, sep_vel] = particle_separation_x(mm-1, mm);
    end
...
%%%%% 2nd plot %%%%%
% this is just the plot in plot_particle_position, but with filling

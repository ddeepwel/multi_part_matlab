...
x1 = nearest_index(xu_new,0.5)+1;
x2 = nearest_index(xu_new,Delta-0.5)-1;

% density field
% set "well" in between particles
c0_new(x1:x2,end-Ny+1:end,:) = repmat(c0(x1,:,:),[x2-x1+1,1,1]);
% set the density extension below the original bottom
if ymin_new < min(yv)
    cd(orig_dir)
    p_mob = check_read_dat('mobile_0');
    cd(direc)
    tind = nearest_index(p_mob.time,time);
    if p_mob.y(tind)-9 < ymin + 7 % the 7 is applicable for diffusion at t/tau = 9
        error('original domain is not deep enough!')
    end
    y_pt = p_mob.y(tind)-9;
    yind = nearest_index(yv_new,y_pt);
    rho_bg = -yc_new(1:yind);
    c0_new(:,1:yind,:) = repmat(rho_bg,[Nx_new,1,Nz]);
end
...
vf_sl = squeeze(vfc_new(:,:,round(Nz/2)));

figure(55)
clf

pcolor(xu_new,yv_new,v_sl' .* (1-vf_sl'))
shading flat
...
v2  = h5read(filename_f, '/Conc/0');
vf2  = h5read(filename_f, '/vfc');
v2_sl = squeeze(v2(:,:,round(Nz/2)));

figure(56)
clf
pcolor(gd2.xc,gd2.yc,v2_sl' .* (1-vf_sl'))
shading flat
colormap(cmocean('balance'))
colorbar
%caxis([-1 1]*0.8)
...
pcolor(gd2.xc, gd2.yc, vdiff' .* (1-vf_sl'))
shading flat
colormap(cmocean('balance'))
colorbar
%caxis([-1 1]*0.8)
